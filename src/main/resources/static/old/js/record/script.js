...
function scoredGoalToRow(goal){
    ...
    players.innerHTML = `G: &nbsp;${goal.scorer?.gameNumber || '--'} &nbsp;|&nbsp; A:&nbsp; ${assists ? assists.join(', ') : '--'} &nbsp;<br> +:&nbsp; ${onIceNumbers.join(', ')}`;
...
function penaltyToRow(penalty){
    ...
    playerTd.textContent = `#${penalty.player?.gameNumber || '--'}`;

    const minutesTd = document.createElement('td');
    minutesTd.innerHTML = `${penalty.minutes} min<br>${penalty?.reason || "---"}`;
